...
class TablaTotalActual extends React.Component {
	render() {
		let totales = [
		...
		for(let i = 0; i < 7; i++){
			...
			Object.keys(this.props[i]).map(key => {
				...
				totales[i]["total"] += this.props[i][key]["mesActual"];
				totalTotales["total"] += this.props[i][key]["mesActual"];
			});
		}
...
export default TablaTotalActual;
